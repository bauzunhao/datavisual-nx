 ...
 */
export const loopShowTooltip = (chart, chartOption, options) => {
  let defaultOptions = {
    interval: 2000,
    loopSeries: false,
    seriesIndex: 0,
  ...
  let highlightChartType = ["pie", "map", "radar"]; // 允许高亮图表类型
  ...
  function cancelHighlight() {
    ...
    if (highlightChartType.includes(tempType)) {
  ...
  function autoShowTip() {
    ...
    function showTip() {
      ...
      if (highlightChartType.includes(chartType)) {
        if (!first) {
          cancelHighlight();
        }
