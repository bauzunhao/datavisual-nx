  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="complaintList" >
      ...
      <el-table-column label="投诉时间" align="center" prop="complaintTime" width="180">
        ...
        <el-table-column label="评价时间" align="center" prop="complaintTime" width="180">
            <template slot-scope="scope">
                <span>{{ parseTime(scope.row.pjTime, '{y}-{m}-{d}') }}</span>
            </template>
        </el-table-column>
        <el-table-column label="评价结果" align="center" prop="type" >
            <template slot-scope="scope">
                <span>  {{ 0 == scope.row.pjResult ? '满意' : 1 == scope.row.pjResult ? '基本满意' : 2 == scope.row.pjResult ? '不满意' :"" }}</span>
            </template>
        </el-table-column>
  ...
  methods: {
    ...
    handleExport() {
          ...
          this.download('grid/complaint/export', {
              ...this.queryParams
          }, `网格评价数据管理_${new Date().getTime()}.xlsx`,{timeout:120000});
      },
  }
};
</script>
