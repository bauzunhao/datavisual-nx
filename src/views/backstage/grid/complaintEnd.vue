<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['grid:complaintEnd:export']"
        >导出</el-button>
      </el-col>
    </el-row>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
                ...
                type="text"
                icon="el-icon-document"
                @click="zjhb(scope.row)"
                v-hasPermi="['grid:complaintEnd:summary']"
            >总结汇报</el-button>
          <el-button
                size="mini"
                type="text"
                icon="el-icon-document"
                @click="hfjl(scope.row)"
                v-hasPermi="['grid:complaintEnd:hf']"
          >回访</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="900px" append-to-body>
      ...
      <el-dialog title="总结汇报" :visible.sync="hbVisible" width="900px" append-to-body>
          <el-form label-position="top" label-width="80px" ref="form" :model="form" :rules="rules">
              <el-form-item label="总结内容" prop="summary">
                  <el-input
                      ...
                      :rows="5"
                      placeholder="请输入内容"
                      v-model="form.summary" maxlength="250" show-word-limit>
                  </el-input>
              </el-form-item>
          </el-form>
          <div style="text-align: center">
              <el-button type="primary" @click="saveHb">提交</el-button>
              <el-button  @click="hbVisible=false">关闭</el-button>
          </div>
      </el-dialog>
      <el-dialog title="回访" :visible.sync="hfVisible" width="900px" append-to-body>
          <el-form label-position="top" label-width="80px" ref="hfForm" :model="hfForm" :rules="hfRules">

              <el-form-item label="回访内容" prop="reserve">
                  <el-input
                      type="textarea"
                      :rows="5"
                      placeholder="请输入内容"
                      v-model="hfForm.reserve" maxlength="250" show-word-limit>
                  </el-input>
              </el-form-item>
          </el-form>
          <div style="text-align: center">
              <el-button type="primary" @click="saveHf">提交</el-button>
              <el-button  @click="hfVisible=false">关闭</el-button>
          </div>
      </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
          summary: [
                { required: true, message: "总结内容不能为空", trigger: "blur" }
         ],
      },
      hfVisible: false,
      hfForm: {
    ...
    handleView(row) {
      ...
      saveHf(){
          this.$refs["hfForm"].validate(valid => {
              if (valid) {
                  updateComplaint(this.hfForm).then(response => {
                      this.$modal.msgSuccess("保存成功");
