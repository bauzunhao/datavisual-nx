  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="900px" append-to-body>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="accept(2)">留存</el-button>
        <el-button type="primary"  @click="accept(0)">网格受理</el-button>
        <el-button type="primary"  @click="innerVisible = true">网格派件</el-button>
        <el-button type="primary" @click="selectPrintOpen">打印登记表</el-button>
        ...
        <el-dialog
            ...
            append-to-body>
            <el-checkbox v-model="zfwChecked" disabled>报送政法委系统</el-checkbox>
            <h4>转派办理意见</h4>
            <el-input
                type="textarea"
                :rows="4"
                placeholder="请输入内容"
                v-model="opinion" maxlength="500" show-word-limit>
            </el-input>
            <div style="text-align: center;margin-top: 20px;">
                <el-button type="primary" @click="accept(3)">提交</el-button>
                <el-button @click="innerVisible = false">取消</el-button>
            </div>
        </el-dialog>
        <!-- 打印登记表 -->
        <el-dialog title="打印登记表" :visible.sync="openPrint" width="50%" append-to-body>
            <div class="page print-div">
                ...
                <table class="tabPrint" cellSpacing="0" cellPadding="0" border="0" style='width:100%'>
                    ...
                    <tr>
                        ...
                        <td colspan="2"><p>{{complaint.name}}</p></td>
                        <td colspan="2" align="center">手机号码</td>
                        <td colspan="4"><p>{{complaint.mobile}}</p></td>
                    </tr>
                    <tr>
                        <td align="center" colspan="2">所属网格社区</td>
                        <td colspan="2"><p>{{complaint.communityName}}</p></td>
                        <td align="center" colspan="2">问题发生地</td>
                        <td colspan="4"><p>{{complaint.addressName}}</p></td>
                    </tr>
                    <tr>
                        <td align="center" colspan="2">投诉时间</td>
                        <td colspan="2"><p>{{complaint.createTime}}</p></td>
                        <td align="center" colspan="2">反映类别</td>
                        <td colspan="4"><p>{{0 ==complaint.type ? '建议' : '投诉'}}</p></td>
                    </tr>
...
<script>
import { listComplaint, getComplaint, delComplaint,acceptComplaint,stateList } from "@/api/backstage/grid/complaint";
...
import printTemplate from "@/api/backstage/grid/twig/complaint.twig";
export default {
  name: "complaintToDoItems",
  data() {
    return {
        innerVisible: false,
        activeName: 'first',
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除网格投诉的数据项？').then(function() {
    ...
    handleExport() {
          ...
          this.download('grid/complaint/export', {
              ...this.queryParams
          }, `网格投诉管理_${new Date().getTime()}.xlsx`,{timeout:120000});
    },
     /** 不予受理*/
    noAccept(row) {
         this.$modal.confirm('是否确认不予受理？').then(function() {
             return acceptComplaint({id:row.id,type:1});
         }).then(() => {
             this.getList();
    ...
    accept(type) {
        ...
        this.$modal.confirm(msg).then(function() {
            return acceptComplaint({id:id,type:type,opinion:opinion});
        }).then(() => {
            this.getList();
            this.$modal.msgSuccess("处理成功");
            this.opinion = "";
