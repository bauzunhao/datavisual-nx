<template>
  <div class="app-container">
      <div class="head">
          <el-card class="box-card">
              <div slot="header" class="clearfix">
                  <span>今年以来重点攻坚件概览</span>
              </div>
              <div class="item">
                  <el-row :gutter="100" class="numlist">
                      <el-col :span="6">
                          <div>
                              <p>{{generalData.todayEnd}}</p>
                              <p>今日办结</p>
                          </div>
                      </el-col>
                      <el-col :span="6">
                          <div>
                              <p>{{generalData.focusTotal}}</p>
                              <p>攻坚总量</p>
                          </div>
                      </el-col>
                      <el-col :span="6">
                          <div>
                              <p>{{generalData.focusHandle}}/{{generalData.focusEndNum}}</p>
                              <p>办理中/已办结</p>
                          </div>
                      </el-col>
                      <el-col :span="6">
                          <div>
                              <p>{{generalData.focusRatio}}</p>
                              <p>攻坚件占比总量</p>
                          </div>
                      </el-col>
                  </el-row>
    ...
    <div>
      ...
      <el-table ref="multipleTable" :data="focusLetterData" tooltip-effect="dark" style="width: 100%" v-loading="loading" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="repeatName" label="初重标志" show-overflow-tooltip></el-table-column>
      ...
      />
        <el-dialog
          ...
          <el-button type="primary" @click="addFocus">立即导入</el-button>
        </span>
        </el-dialog>

      <el-dialog title="统计" :visible.sync="reportVisible">
        <el-table :data="focusReportList" :summary-method="getSummaries" show-summary>
          <el-table-column label="地区" align="center" prop="zoneName"/>
          <el-table-column label="重点工作各地市分布" align="center">
...
<script>
    ...
    import { listFocusWork,addFocusWork,delFocusWorkByLetterCode,listFocusReportData,getFocusSummaryData} from "@/api/backstage/work/focus";
    export default {
        name: "Focus",
        ...
        methods: {
            ...
            getList() {
              this.loading = true;
              listFocusWork(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
                  this.focusLetterData = response.rows;
           ...
           /** 导入弹窗*/
           addFocus(){
             const letterCodesValue = this.letterCodes.trim().replaceAll("\n", ";")
               if(letterCodesValue){
                 addFocusWork({'letterCode':letterCodesValue}).then(response => {
          ...
          handleDelete(row) {
            ...
            this.$modal.confirm('是否确认删除信访件编号为"' + letterCode + '"的数据项？').then(function() {
              return delFocusWorkByLetterCode(letterCode);
            }).then(() => {
              this.getList();
          ...
          handleExport() {
            ...
            this.download('work/focus/export', {
              ...this.queryParams
            }, `重点工作管理_${new Date().getTime()}.xlsx`,{timeout:120000})
          },
          /** 统计按钮操作 */
          reportQuery() {
            this.reportVisible = true;
            this.loading = true;
            listFocusReportData(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
              this.focusReportList = [{}];
              if (response.data) {
                this.focusReportList = response.data;
              }
              this.loading = false;
            });
          },
          ...
          /** 初件汇总数据查询 */
          focusSummaryDataQuery() {
            getFocusSummaryData().then(response => {
              if (response.data) {
