<template>
    <div class="app-container">
                ...
                <el-col :span="12">
                    <el-form-item label="标记类型：" class="flexstart">
                        <el-radio-group v-model="queryParams.markType" size="small">
                            ...
                            <el-radio-button label="3">最多访一次</el-radio-button>
                            <el-radio-button label="4">专项件</el-radio-button>
                        </el-radio-group>
                    </el-form-item>
                </el-col>
            </el-row>
        ...
        <el-row :gutter="10" class="mb8">
            ...
            <el-col :span="1.5">
                <el-button type="primary" plain icon="el-icon-pie-chart" size="mini" @click="reportQuery">统计
                </el-button>
            </el-col>
        ...
        <div>
            <el-table ref="multipleTable" :data="threatenLetterData" tooltip-effect="dark" style="width: 100%" v-loading="loading">
                ...
                <el-table-column label="操作" align="center" width="160" class-name="small-padding fixed-width">
                    <template slot-scope="scope">
                        <el-button
                            ...
                            size="mini"
                            type="text">
                            <router-link :to="{ path: '/workbenches/detailcomp/' + scope.row.objectId }">详情
                            </router-link>
                        </el-button>
                    </template>
                </el-table-column>
            ...
            <el-dialog title="统计" :visible.sync="dialogTableVisible">
                <el-table :data="threatenReportList" :summary-method="getSummaries" show-summary>
                    <el-table-column label="地区" align="center" prop="zoneName"/>
                    <el-table-column label="扬言件各地市分布" align="center">
...
import {listThreatenData, listThreatenReportData, getThreatenSummaryData} from "@/api/backstage/work/threaten";
...
export default {
    ...
    methods: {
        ...
        getList() {
            this.loading = true;
            listThreatenData(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
                    this.threatenLetterData = response.rows;
        ...
        handleExport() {
            ...
            this.download('work/source/threaten/export', {
                ...this.queryParams
            }, `扬言件管理_${new Date().getTime()}.xlsx`, {timeout: 120000});
        },
        /** 统计按钮操作 */
        reportQuery() {
            this.dialogTableVisible = true;
            this.loading = true;
            listThreatenReportData(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
                this.threatenReportList = [{}];
                if (response.data) {
                    this.threatenReportList = response.data;
                }
                this.loading = false;
            });
        },
        ...
        /** 扬言件汇总数据查询 */
        threatenSummaryDataQuery() {
            getThreatenSummaryData().then(response => {
                if (response.data) {
