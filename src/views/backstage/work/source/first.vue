<template>
  <div class="app-container">
      ...
      <el-card class="box-card">
          ...
          <div class="item">
              <el-row :gutter="100" class="numlist">
                  ...
                  <el-col :span="6">
                      <div>
                          <p>{{generalData.nationNum}}/{{generalData.provinceNum}}/{{generalData.cityNum}}</p>
                          <p>国家/自治区/全地市</p>
                      </div>
                  </el-col>
                  <el-col :span="6">
    ...
    <div>
      <el-table ref="multipleTable" :data="firstLetterData" tooltip-effect="dark" style="width: 100%" v-loading="loading" >
      ...
      <el-dialog title="统计" :visible.sync="dialogTableVisible">
        <el-table :data="firstReportList" :summary-method="getSummaries" show-summary >
          <el-table-column label="地区" align="center" prop="zoneName"/>
          <el-table-column label="初件各地市分布" align="center">
...
<script>
  ...
  import { listFirstData,listFirstReportData,getFirstSummaryData } from "@/api/backstage/work/first";
  export default {
    name: "First",
    ...
    methods: {
      ...
      getList() {
        this.loading = true;
        listFirstData(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
            this.firstLetterData = response.rows;
      ...
      handleExport() {
        if(this.total > 5000){
         ...
         this.download('work/source/first/export', {
              ...this.queryParams
            }, `初件管理_${new Date().getTime()}.xlsx`,{timeout:120000});
      },
      /** 统计按钮操作 */
      reportQuery() {
        this.dialogTableVisible = true;
        this.loading = true;
        listFirstReportData(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
          this.firstReportList = [{}];
          if (response.data) {
            this.firstReportList = response.data;
          }
          this.loading = false;
        });
      },
      ...
      /** 初件汇总数据查询 */
      firstSummaryDataQuery() {
        getFirstSummaryData().then(response => {
          if (response.data) {
